...
<div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100/60 p-8 max-w-4xl mb-10">
    ...
    <div class="mb-8 pb-6 border-b border-slate-100 flex items-center justify-between">
        <div class="flex items-center gap-4">
            <div
                class="w-12 h-12 bg-amber-500 text-white rounded-xl flex items-center justify-center shadow-lg shadow-amber-500/20">
                <i data-lucide="user-cog" class="w-6 h-6"></i>
            </div>
            <div>
                <span
                    class="inline-block bg-amber-100 text-amber-700 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md mb-1.5">
                    ✏️ Edit Data
                </span>
                <h3 class="text-xl font-extrabold text-slate-800 tracking-tight">Ubah Data Pegawai</h3>
            </div>
        </div>
    </div>
    ...
    <form action="/master-pegawai/{{ $master_pegawai->id }}" method="POST" class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="nomor_whatsapp"
                    value="{{ old('nomor_whatsapp', $master_pegawai->nomor_whatsapp) }}"
            ...
            <div>
                ...
                <select name="unit_kerja_id" required
                    ...
                    <option value="{{ $unit->id }}" {{ old('unit_kerja_id', $master_pegawai->unit_kerja_id) == $unit->id
                        ? 'selected' : '' }}>
                        {{ $unit->nama_unit }}
                    </option>
                    @endforeach
            ...
            <div>
                ...
                <select name="role" id="role_select" required
                    ...
                    <option value="staf" {{ old('role', $master_pegawai->role) == 'staf' ? 'selected' : '' }}>Staf /
                        Karyawan Pelaksana (Akses Mobile Saja)</option>
                    <option value="kepala_unit" {{ old('role', $master_pegawai->role) == 'kepala_unit' ? 'selected' : ''
                        }}>Kepala Unit Kerja</option>
                    <option value="penanggung_jawab" {{ old('role', $master_pegawai->role) == 'penanggung_jawab' ?
                        'selected' : '' }}>Penanggung Jawab Shift</option>
                    <option value="manajer" {{ old('role', $master_pegawai->role) == 'manajer' ? 'selected' : ''
                        }}>Manajer (Multi-Unit)</option>
                    <option value="direktur" {{ old('role', $master_pegawai->role) == 'direktur' ? 'selected' : ''
                        }}>Direktur (Akses Global)</option>
                    <option value="hrd" {{ old('role', $master_pegawai->role) == 'hrd' ? 'selected' : '' }}>HRD /
                        Payroll</option>
            ...
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Kata Sandi Baru <span
                        class="text-xs text-slate-400 font-normal">(Kosongkan jika tidak ingin mengubah)</span></label>
        ...
        <!-- UNIT YANG DIKELOLA (Muncul jika role = Kepala Unit, PJ, atau Manajer) -->
        ...
        <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
            ...
            <button type="submit"
                class="bg-amber-500 hover:bg-amber-600 text-white px-8 py-3.5 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg shadow-amber-500/20 transition-all">
